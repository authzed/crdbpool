...
func TestMultiNodeClusterHealth(t *testing.T) {
	...
	t.Run("health tracker detects connected node", func(t *testing.T) {
		...
		require.Greater(t, healthyCount, 0, "Should detect at least one healthy node")
		t.Logf("Health tracker detected %d healthy node(s)", healthyCount)
	})

	t.Run("node failure detection when connected node stops", func(t *testing.T) {
		// Create health tracker connected to node 1
		healthTracker, err := NewNodeHealthChecker(cluster.GetNodeURI(1))
		require.NoError(t, err)
		...
		// Wait for initial health discovery
		time.Sleep(5 * time.Second)
		initialCount := healthTracker.HealthyNodeCount()
		require.Greater(t, initialCount, 0, "Should start with at least one healthy node")

		// Stop the node we're connected to (node 1, index 1)
		err = cluster.StopNode(1)
		require.NoError(t, err)

		// Wait for health tracker to detect the failure
		time.Sleep(10 * time.Second)

		// Should now detect fewer healthy nodes (likely 0 since we stopped the node we connect to)
		healthyCount := healthTracker.HealthyNodeCount()
		t.Logf("Health count after stopping connected node: %d (was %d)", healthyCount, initialCount)

		// The health tracker should either have 0 nodes (can't connect) or maintain the last known count
		// depending on the implementation's behavior when connection fails
		require.True(t, healthyCount <= initialCount, "Should not have more healthy nodes after stopping one")
	})
}
